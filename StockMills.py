...
def can_remove_this_stone(board, pos, opp):
    ...
    Stone removal rule:
      ...
      - If it's in a mill, only removable if ALL opponent stones are in mills.
    """
...
def possible_removals(board, opp):
    """
    Return a list of positions of opponent stones that can be removed,
    obeying the removal rule.
    """
    removables = []
...
def forms_mill_after_placement(board, point, color):
    """Temporarily place a stone at 'point' and check for a mill."""
    old = board[point]
    board[point] = color
...
def forms_mill_after_move(board, source, target, color):
    """Temporarily move stone from 'source' to 'target' and check for a mill."""
...
def generate_moves(state):
    ...
    # 1) Placement moves if stones remain in hand.
    if in_hand > 0:
...
def apply_move(state, source, target, remove):
    """
    Apply the move (source -> target, remove) and return a new state.
    Adjusts stones in hand and switches the current player.
    """
...
def is_terminal(state):
    """
    Terminal if a player's stone count (on board) is ≤2 when no stones remain in hand,
...
def utility(state):
    ...
    Return terminal utility:
      +infinity if current player wins,
      -infinity if loses,
      0 otherwise.
    """
    board = state["board"]
...
def evaluate(state):
    """
    Heuristic evaluation for non-terminal states based on:
      - Stone count difference
      - Mobility (number of moves)
      - Center control
      - Bonus for stones in mills
    """
    board = state["board"]
    color = state["current_player"]
    opp = "blue" if color == "orange" else "orange"
...
def evaluate_or_utility(state):
    """Return utility if terminal, else heuristic evaluation."""
...
def iterative_deepening(state, max_iter_depth, time_limit):
    """
    Iterative deepening from depth 1 to max_iter_depth, stopping if time runs out.
    """
    start = time.time()
...
def main():
    ...
    while True:
        try:
            ...
            # Set current player to our color before our move.
